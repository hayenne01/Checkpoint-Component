  ...
  },
{
 ...
 return (
   <section>
     <h2 className="Prod">List Of Products</h2>
      <div className="list-Products"></div>
      {products.map((product) => (
        <Product product={product} key={product.id}  />
      ))}
              <h2 className="title">
        List Of products that their category is Men's clothing and their price
        greater tha 50:{" "}
      </h2>
      {products
        .filter(
          (product) =>
            product.price > 50 && product.category === "men's clothing"
        )
        .map((product) => (
          <Product product={product} key={product.id} />
        ))}
